...
public class Undo_Redo {
	...
	public Undo_Redo(int sel,int day,int month,int year,int week_day,int total_size,int array_position,Hashtable<Integer,Employee> employee) {
		set_old_sel(sel);
		set_old_day(day);
		set_old_month(month);
		set_old_year(year);
		set_old_week_day(week_day);
		set_oldHT(employee,total_size);
		set_old_array_position(array_position);
	}
	
	public static void Save(int sel,int day,int month,int year,int week_day,int total_size,int array_position,Hashtable<Integer,Employee> employee) {
		Undo_Redo undo_redo = new Undo_Redo(sel,day,month,year,week_day,total_size,array_position,employee);
		Undo.push(undo_redo);
	}
	
	public static void SaveRedo(int sel,int day,int month,int year,int week_day,int total_size,int array_position,Hashtable<Integer,Employee> employee) {
		Undo_Redo undo_redo = new Undo_Redo(sel,day,month,year,week_day,total_size,array_position,employee);
		Redo.push(undo_redo);
	}
	...
	}	public Hashtable<Integer,Employee> get_old_hashtable() {
		return old_HT;
	}
	public void set_oldHT(Hashtable<Integer,Employee> HT,int total_size) {
